...
add_button = ttk.Button(Program,bootstyle="success", text="Add Product", command=add_product)
add_button.grid(row=8, columnspan=6, pady=30)
...
remove_button.grid(row=8, columnspan=9, pady=30)
def sell_product():
    selected_items = Tapbar.selection()
    for item in selected_items:
        product_name= Tapbar.item(item, 'text')
...
sell_button = ttk.Button(Program, bootstyle="success",text="Sell Product", command=sell_product)
